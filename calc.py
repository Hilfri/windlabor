...
class Calculator():
    ...
    def change_header_names(self, df):
        ...
        return df
    def get_formatted_data(self, files_list):
        time_data = []
        rpm_data = []
        print(files_list)
        for filename in files_list:
            #day, time, airgap, load, mps, id, iteration = filename.replace(".txt","").split('_')
            mps= 10000

            df = self.load_measurements('messwerte/1406_1625.txt')
            ...
            for i in [1,2,3,'dc']:
                time_data.append(
                    get_graph(new_df, 'time', 'voltage_{}'.format(i))
                    )
                time_data.append(
                    get_graph(new_df, 'time', 'current_{}'.format(i))
                    )
                time_data.append(
                    get_graph(new_df, 'time', 'eff_volt_{}'.format(i))
                    )
                time_data.append(
                    get_graph(new_df, 'time', 'eff_curr_{}'.format(i))
                    )
            time_data.append(
